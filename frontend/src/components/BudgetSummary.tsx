...
export function BudgetSummary() {
    ...
    const fetchData = async () => {
      try {
        ...
        const totalBudget = budgetResponse.data
          ...
          .reduce(
            (acc: number, category: { monthly_budget: number }) =>
              acc + category.monthly_budget,
            0
          );
  ...
  const spentPercentage =
    total > 0 ? Math.min((remaining / total) * 100, 100) : 0;
  const radius = 58;
  const circumference = 2 * Math.PI * radius;
  const dashOffset =
    total > 0
      ? Math.max(circumference * (1 - spentPercentage / 100), 0)
      : circumference;
