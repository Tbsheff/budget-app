  ...
  const onPlaidSuccess = useCallback(
    async (publicToken: string, metadata: any) => {
      try {
        ...
        setError(
          "Unable to link your bank account. This could be due to a connection issue or temporary service disruption."
        );
        toast({
  ...
  const handlePlaidExit = useCallback(
    (err: any) => {
      ...
      if (err && err.error_code && err.error_code !== "user_closed") {
        ...
        setError(
          `Connection issue: ${err.display_message || err.error_message || "Unknown error"}`
        );
      }
      if (onExit) {
        onExit();
